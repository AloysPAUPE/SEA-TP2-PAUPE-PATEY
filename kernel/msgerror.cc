...
SyscallError::SyscallError() {
  ...
  msgs[INEXIST_FILE_ERROR] =
      (char *) "file %s does not exist or is a directory\n";
  msgs[INEXIST_DIRECTORY_ERROR] =
  ...
  msgs[WRONG_FILE_ENDIANESS] = (char *) "Incorrect code endianess\n";
...
//-----------------------------------------------------------------
void
SyscallError::SetMsg(char *about, int num) {
...
//-----------------------------------------------------------------
const char *
SyscallError::GetFormat(int num) {
  if ((num < 0) || (num >= NUMMSGERROR))
    num = INC_ERROR;
  if (msgs[num] == NULL)
    num = INC_ERROR;
...
//-----------------------------------------------------------------
void
SyscallError::PrintLastMsg(DriverConsole *cons, char *ch) {
